...
func cmdCMSINITBYDIM(args []string) []byte {
	...
	cols, err := strconv.ParseUint(args[1], 10, 32)
	if err != nil {
		return Encode(errors.New(fmt.Sprintf("Cols must be integer %s", args[1])), false)
	}

	rows, err := strconv.ParseUint(args[2], 10, 32)
	if err != nil {
...
func cmdCMSINCRBY(args []string) []byte {
	...
	for i := 1; i < len(args); i += 2 {
		...
		count := cms.Increase(item, uint32(value))
